...
if (isset($_POST['accesskey']) && isset($_POST['category_id'])) {
    ...
    if ($access_key_received == $access_key) {
        ...
        $sql = "SELECT count(id) as total FROM products WHERE `status`=1 and category_id=" . $subcategory_id;
        $db->sql($sql);
        $res = $db->getResult();
        foreach ($res as $row) {
            $total = $row['total'];
        }
        $sql = "SELECT p.*,(SELECT " . $price . " FROM product_variant pv WHERE pv.product_id=p.id) as price FROM products p WHERE `status`=1 and category_id='" . $subcategory_id . "' $sort LIMIT $offset, $limit";
        $db->sql($sql);
        $res = $db->getResult();
        $product = array();
