...
if (isset($_POST['place_order']) && isset($_POST['user_id']) && !empty($_POST['product_variant_id'])) {
	...
	for ($i = 0; $i < count($item_details); $i++) {
		...
		$tax_percentage = $item_details[$i]['tax_percentage'];  //($$tax_percentage / 100) * $discounted_price
// Rs. 100 - Tax 5% Rs. 5 - Qty 1 - Subtotal ( 100+5 ) x 1 = 105
	...
	// 		$_SESSION['checkout']['order_id'] = $order_id;
		...
		// echo "item details ".print_r($item_details);
		for ($i = 0; $i < count($item_details); $i++) {
			$product_id = $item_details[$i]['product_id'];
			// $unit = (float) filter_var( $item_details[$i][2], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION ) ; // float(55.35) 
			$measurement = $item_details[$i]['measurement'];
			...
			$item_data1[] = array('name' => $item_details[$i]['name'],'unit' =>  $item_details[$i]['measurement'] . " " . $item_details[$i]['measurement_unit_name'],'qty' => $quantity,'subtotal' => ($price * $quantity));

		}		
		$message .= "<b>OTP : </b>" . $otp ."<b>Total Amount : </b>" . $total . " <b>Delivery Charge : </b>" . $delivery_charge . " <b>Tax Amount : </b>" . $tax_amount . " <b>Discount : </b>" . $discount . " <b>Wallet Used : </b>" . $wallet_balance . " <b>Final Total :</b>" . $final_total;
		$message .= "<br>Payment Method : " . $payment_method;

		$order_data = array('total_amount' => $total,'delivery_charge' => $delivery_charge,'tax_amount' => $tax_amount,'discount' => $discount,'wallet_used' => $wallet_balance,'final_total' => $final_total,'payment_method' => $payment_method,'address' => $address,'user_msg' => $user_msg,'otp_msg' => $otp_msg,'otp' => $otp_number);
		// echo "****order_data".print_r($order_data);
		
		$message .= "<br><br>Thank you for placing an order with us!<br><br>You will receive future updates on your order via Email!";
		send_email_with_template($to, $subject,$item_data1,$order_data);
		// send_email($to, $subject, $message);
		
		// send_email_with_template("infinitietechnologies03@gmail.com","Order received successfully!!!",$item_data1,$order_data);
