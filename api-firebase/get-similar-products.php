...
if (isset($_POST['accesskey']) && isset($_POST['get_similar_products']) && !empty($_POST['get_similar_products'])) {
	...
	if ($access_key_received == $access_key) {
		...
		$sql = "SELECT count(id) as total FROM products where p.id != $product_id and category_id = $category_id and p.status = 1 ORDER BY $order LIMIT $offset,$limit";
		$db->sql($sql);
        $total1 = $db->getResult();
        
		$sql = "SELECT p.*,(SELECT MIN(pv.price) FROM product_variant pv WHERE pv.product_id=p.id) as price FROM products p where p.id != $product_id and category_id = $category_id and p.status = 1 ORDER BY $order LIMIT $offset,$limit";
		$db->sql($sql);
        $res = $db->getResult();
        if (!empty($res)) {
            foreach ($res as $row) {
